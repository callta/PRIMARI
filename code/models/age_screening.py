...
class AgeScreening(Save):

    """Build cohort of age-based screening using MRI-first approach
    to diagnosis."""
    ...
    def _run_model(self, params):
        ...
        for year in np.arange(55, 70):
            ...
            if year < 55:
                ...
                pca_incidence_age = params.adjusted_incidence[:, year-45:]
                pca_mortality_age = params.adjusted_mortality[:, year-45:]

                # Proportion of cancers detected by screening at a localised / advanced stage
                stage_screened_local = params.ns_localised_stage[:, year-45:]
                stage_screened_adv = params.ns_advanced_stage[:, year-45:]

            if year > 54:
                # PCa incidence (adjusting for change in clinically (in)significant cancers)
                pca_incidence_age = params.adjusted_incidence.copy()
                pca_incidence_age[:,10:25] = (pca_incidence_age[:,10:25].T * params.rr_incidence_adjusted[year-45,:]).T
                ...
                # Death from PCa in screened
                pca_mortality_age = params.adjusted_mortality.copy()
                ...
                # Proportion of cancers detected by screening at a localised / advanced stage
                stage_screened_local = params.sc_localised_stage[:, year-45:]
                stage_screened_adv = params.sc_advanced_stage[:, year-45:]
            ...
            if year > 54:
                ...
                pca_misclassified = pca_incidence_screened * p_misclassified # Number of cancers miclassified
            ...
            for i in range(1, total_cycles):
               ...
               # Cohorts, numbers healthy, incident & prevalent cases
                ...
                pca_incidence_sc[:,i] = (healthy_sc[:,i] * pca_incidence_age[:,i]) + pca_misclassified_array[:,i]

                if year > 54:
                    if i < length_screen:
                        # Incidence
                        pca_incidence_screened[:,i] = healthy_sc[:,i] * pca_incidence_age[:,i]
                        pca_incidence_post_screening[:,i] = 0

                        # Misclassified
                        pca_misclassified[:,i] = pca_incidence_screened[:,i] * p_misclassified[:,i]

                        # add these in equal spread over the params.mst
                        # first work out that year's additional number divided by params.mst
                        # then add to the array over the following years
                        ...
                        pca_incidence_post_screening[:,i] = healthy_sc[:,i] * pca_incidence_age[:,i]
                ...
                # Incidence in the no screening group
                ...
                if year > 54:
                    costs_tx_screened[:,i] = ((pca_incidence_screened[:,i]
                                               * stage_screened_local[:,i].T
                                               * tx_costs_local.T).sum(axis=0)

                                              + (pca_incidence_screened[:,i]
                                                 * stage_screened_adv[:,i].T
                                                 * tx_costs_adv.T).sum(axis=0)) # cost of screen-detected cancers
            ...
            # Overdiagnosed cases
            overdiagnosis_age = pca_incidence_screened * params.p_overdiagnosis_psa.T[:, year-45:]
            ...
            # no healthy_ns if uptake is 100%, as all will be screened
            # therefore remove health_ns here to prevent erroneous negative number
            ...
            # PSA testing in non-screened arm
            ...
            # PSA testing in screened arm (PSA screening every four years)
            if year < 55:
                # Assuming all cancers are clinically detected as these cohorts
                # are not eligible for screening (hence params.p_biopsy_ns)
            ...
            if year > 54:
                # Number of healthy individuals eligible for screening
            ...
            if year < 55:
                # Assuming all cancers are clinically detected as these cohorts
                # are not eligible for screening (hence params.p_biopsy_ns)
                # Number of MRI are equal to the number of biopsies
                # (prior to adjustment for biopsies with an MRI-first approach)
                n_mri_sc_age = pca_incidence_sc / params.p_biopsy_ns[:, year-45:]
            ...
            if year > 54:
                ...
                # params.uptake_mri = proportion screening uptake: set as 1 except in sensitivity analysis.
                n_mri_screened_age = ((n_psa_tests_screened_age * params.p_mri[:, year-45:]) * params.uptake_mri)
                cost_mri_screened_nodiscount_age = n_mri_screened_age * np.array([params.cost_mri]).T

                # Assuming all cancers are clinically detected in the post-screening phase
                n_mri_post_screening_age = pca_incidence_post_screening / params.p_biopsy_ns[:, year-45:]
            ...
            n_mri_ns_age = pca_incidence_ns / params.p_biopsy_ns[:, year-45:]
            ...
            if year < 55:
                n_biopsies_sc_age = n_mri_sc_age * params.n_biopsies_primari[:, year-45:]
                cost_biopsy_sc_nodiscount_age = ((n_biopsies_sc_age * params.cost_biopsy[:, year-45:])
                                                 * params.relative_cost_clinically_detected[:, year-45:])

            if year > 54:
                # Screen-detected cancers
                n_biopsies_screened_age = n_mri_screened_age * params.n_biopsies_primari[:, year-45:]
                cost_biopsy_screened_nodiscount_age = n_biopsies_screened_age * params.cost_biopsy[:, year-45:]

                # Assuming all cancers are clinically detected in the post-screening phase
                n_biopsies_post_screening_age = n_mri_post_screening_age * params.n_biopsies_primari[:, year-45:]
                cost_biopsies_post_screening_nodiscount_age = ((n_biopsies_post_screening_age * params.cost_biopsy[:, year-45:])
                                                               * params.relative_cost_clinically_detected[:, year-45:])
            ...
            n_biopsies_ns_age = n_mri_ns_age * params.n_biopsies_primari[:, year-45:]
            ...
            cohort = pd.DataFrame({
                ...
                'pca_misclassified': mean(pca_misclassified),
            ...
            outcomes = pd.DataFrame({
                ...
                'pca_misclassified': [total(pca_misclassified)],
